...
class ConvexSopRepository(
    ...
    private val client: ConvexClient = ConvexClient(baseUrl),
) : SopRepository {
    override fun observeByPartNumber(partNumber: String): Flow<ProcedureDetail?> {
        val function = object : ConvexFunction.Query<ByPartArgs, ProcedureByPartResponse> {
            override val identifier = "procedures:getByPartNumber"
            override val args = ByPartArgs(partNumber)
        }
        return client.query(function).mapSuccess { response ->
    ...
    override fun observeSearchParts(query: String): Flow<List<PartSearchResult>> {
        val function = object : ConvexFunction.Query<SearchArgs, List<SearchItemResponse>> {
    ...
    override fun observeVersions(procedureId: String): Flow<List<SopVersion>> {
        val function = object : ConvexFunction.Query<ProcedureIdArgs, List<VersionListItemResponse>> {
    ...
    override fun observeVersion(versionId: String): Flow<SopVersion?> {
        val function = object : ConvexFunction.Query<VersionIdArgs, VersionResponse?> {
    ...
    override fun observeCurrentUserEmail(): Flow<String?> {
        val function = object : ConvexFunction.Query<EmptyArgs, CurrentUserResponse> {
    ...
    override suspend fun saveNew(partNumber: String, title: String, body: String, photos: List<PhotoPayload>) {
        val function = object : ConvexFunction.Mutation<CreateArgs, MutationResult> {
    ...
    override suspend fun saveEdit(procedureId: String, title: String, body: String, photos: List<PhotoPayload>) {
        val function = object : ConvexFunction.Mutation<EditArgs, MutationResult> {
...
private fun <I, O> Flow<ConvexResponse<I>>.mapSuccess(mapper: (I) -> O): Flow<O> {
    return map { response ->
        when (response) {
...
private fun ConvexResponse<*>.requireSuccess() {
...
@Serializable
private data class EmptyArgs(val placeholder: String? = null)

@Serializable
private data class SearchArgs(val partNumberQuery: String)

@Serializable
private data class ByPartArgs(val partNumber: String)

@Serializable
private data class ProcedureIdArgs(val procedureId: String)

@Serializable
private data class VersionIdArgs(val versionId: String)
...
@Serializable
private data class SavePhoto(
    val storageId: String,
    val description: String? = null,
)

@Serializable
private data class SearchItemResponse(
    val partNumber: String,
...
@Serializable
private data class ProcedureByPartResponse(
...
@Serializable
private data class PartResponse(val partNumber: String)

@Serializable
private data class ProcedureResponse(@SerialName("_id") val id: String)

@Serializable
private data class VersionListItemResponse(
...
private fun VersionResponse.toSopVersion(): SopVersion {
    return SopVersion(
        id = id,
